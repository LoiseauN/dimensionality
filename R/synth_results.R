...
synth_results<- function(trait_df,single_df,miss_final_df,dim_df){
  ...
  quanti_perc <- (sum(do.call(c,lapply(1:ncol(trait_df),function(i){is.numeric(trait_df[,i])})))/ncol(trait_df))*100
  
  #Dimension results
  Nb_dim_AUC_0.5  <-  dim_df$dim[tail(which(dim_df$AUC < 0.5),n=1)+1]
  if(is.integer(Nb_dim_AUC_0.5) && length(Nb_dim_AUC_0.5) == 0) Nb_dim_AUC_0.5   <- NA
  Nb_dim_AUC_0.7   <- dim_df$dim[tail(which(dim_df$AUC < 0.7),n=1)+1]
  
  #Nb_dim_MAD_0.5  <-  dim_df$dim[tail(which(dim_df$MAD < 0.5),n=1)+1]
  #Nb_dim_MAD_0.7   <- dim_df$dim[tail(which(dim_df$MAD < 0.7),n=1)+1]
  ...
  #Nb_miss_MAD_0.5  <-  miss_final_df$miss_percent[tail(which(miss_final_df$MAD > 0.5),n=1)]
  #Nb_miss_MAD_0.7   <- miss_final_df$miss_percent[tail(which(miss_final_df$MAD > 0.7),n=1)]
  ...
  #Perclost_MAD_50perctrait <-  (1-miss_final_df[miss_final_df$miss_percent==0.5,]$MAD)*100
  ...
  res<- data.frame(S                  = S,
                   ...
                   #Nb_dim_MAD_0.5     = Nb_dim_MAD_0.5,
                   #Nb_dim_MAD_0.7     = Nb_dim_MAD_0.7,
                   ...
                   #Perc_miss_MAD_0.5    = as.numeric(as.character(Nb_miss_MAD_0.5)) * 100,
                   #Perc_miss_MAD_0.7    = as.numeric(as.character(Nb_miss_MAD_0.7))* 100,
                   ...
                   #Percentage_lostMAD_depleted0.5 = Perclost_MAD_50perctrait,
                   Nb_single          = Nb_single,
