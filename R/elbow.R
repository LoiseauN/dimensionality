#' @title Elbox Method to Detect Infection Point in a Concave Curve
#'
#' @description This function implements the Elbox rule to detect infection
#'   point in a concave curve.
#'
...
elbow <- function(data, xmin = NULL, xmax = NULL, ymin = NULL, ymax = NULL) {
  ...
  data[ , "SelectedorNot"] <- NA
   ...
   for (i in 1:nrow(data)) {
     if(data$Axe[i] <= data[which.max(data[ , "benefits"]), ]$Axe) {data[i, "SelectedorNot"] <- "Selected"
